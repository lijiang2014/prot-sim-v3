  ...
  {{message}}
    <el-row>
      <el-col :span="formData?.render.width" :offset="formData?.render.offset">
        ...
        <el-form :model="runtimeForm" :rules="runtimeRules" ref="topFormRef" label-width="120px" class="demo-ruleForm"
        ...
        <el-form label-position="top" label-width="120px" :model="message" ref="bottomFormRef">
          ...
          <el-form-item v-for="item in formData?.render.data" :label="item.label" :prop="item.id">
...
const submitForm = async () => {
  ...
  let pass = true
  // check forms
  await topFormRef.value?.validate((valid, fields) => {
    if (valid) {
      console.log('sumit!')
    } else {
